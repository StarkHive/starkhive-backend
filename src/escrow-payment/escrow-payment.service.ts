...
export class EscrowPaymentService {
  ...
  async createEscrowPayment(data: {
    ...
    const payment = this.escrowPaymentRepo.create({
      ...
      totalAmount,
      splitType,
      splits: splits.map(s => this.splitRepo.create({
  ...
  async releaseSplit(splitId: string, details: any): Promise<EscrowPaymentSplit> {
    const split = await this.splitRepo.findOne({ where: { id: splitId }, relations: ['escrowPayment', 'auditLogs'] });
    ...
    // Mark as completed (could be partial logic here)
  ...
  async getAuditLogs(splitId: string): Promise<EscrowPaymentAuditLog[]> {
    return this.auditRepo.find({ where: { split: { id: splitId } }, order: { timestamp: 'DESC' } });
  }
}
